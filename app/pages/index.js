...
const Home = () => {
  return (
    <div>
        ...
        <p>Ceci est la page d'accueil de mon blog. Découvrez nos derniers articles.</p>
        <Header> </Header>
    </div>
  )
}
...
const Home = () => {
  ...
  useEffect(() => {
    const randPokeID = (min, max) => Math.floor(Math.random() * (max - min + 1) + min);
  ...
  return (
    <div className="min-h-screen bg-cover h-14 bg-gradient-to-b from-indigo-950 to-slate-950">
      ...
      <div >
        <p className="absolute top-40 right-32 font-mono font-extrabold text-2xl text-white">Empower Your Voice, Elevate Your Game</p> 
        <p className="absolute top-48 right-32 font-mono font-extrabold text-2xl text-white">Where Passionate Minds Unite to Share the Essence of Esports</p>          
      </div>
    </div>
